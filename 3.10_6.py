...
class FuncModel(nn.Module):
    def __init__(self):
        ...
        self.layer = nn.Linear(5, 1, bias=True)
...
for _e in range(epochs):
    ...
    Q_val /= count_val  # сохранить средние потери, вычисленные по выборке валидации, в переменной Q_val
...
model.eval() # перевести модель в режим эксплуатации
p = model(data_x)  # выполнить прогноз модели по всем данным выборки (ds.data)
