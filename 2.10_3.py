...
class IrisClassModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.layer1(x))# через функцию активации torch.relu()
        # через второй слой
        x = self.layer2(x)# полученный (вычисленный) тензор возвращается


torch.manual_seed(11)

model = IrisClassModel() # создать модель IrisClassModel с числом входов 4 и числом выходов 3
model.eval() # перевести модель в режим обучения
...
for _e in range(epochs): # итерации по эпохам
    for x_train, y_train in train_data:
        ...
        loss = loss_func(predict, y_train) # вычислить значение функции потерь maybe a
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
Q = # вычислить долю верных классификаций (сохранить, как вещественное число, а не тензор)
